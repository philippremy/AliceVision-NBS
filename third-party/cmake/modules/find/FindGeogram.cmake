...
#   Geogram_VERSION      - The version of Geogram, if any
...
#          Pierre Moulon
#          Bruno Levy
#          Philipp Remy (for version parsing)
...
# Attempt to extract a version number
if(EXISTS ${GEOGRAM_INSTALL_PREFIX})
  # The geogram1.pc file is located at:
  # <INSTALL_PREFIX>/lib/pkgconfig/geogram1.pc
  # However, we will just GLOB for maximum compatibility
  file(GLOB_RECURSE GEOGRAM1_PKGCONFIG_FILES LIST_DIRECTORIES FALSE CONFIGURE_DEPENDS "${GEOGRAM_INSTALL_PREFIX}/*geogram1.pc")
  if(DEFINED GEOGRAM1_PKGCONFIG_FILES)
    foreach(GEOGRAM1_PKGCONFIG_FILE IN LISTS GEOGRAM1_PKGCONFIG_FILES)
      # Try to find the version part in the file
      file(STRINGS "${GEOGRAM1_PKGCONFIG_FILE}" GEOGRAM1_PKGCONFIG_VERSION_STRING REGEX "^[Vv]ersion:[ \t]*([0-9]+(\\.[0-9]+(\\.[0-9]+)?)?)$")
      if(DEFINED GEOGRAM1_PKGCONFIG_VERSION_STRING)
        string(REGEX REPLACE "^[Vv]ersion:[ \t]*([0-9]+(\\.[0-9]+(\\.[0-9]+)?)?)$" "\\1" GEOGRAM1_VERSION "${GEOGRAM1_PKGCONFIG_VERSION_STRING}")
        if(DEFINED GEOGRAM1_VERSION)
          set(Geogram_VERSION ${GEOGRAM1_VERSION} CACHE STRING "The Geogram version found" FORCE)
        endif()
      endif()
    endforeach()
  else()
    message(STATUS "Unable to parse Geogram version because no geogram1.pc file was found in ${GEOGRAM_INSTALL_PREFIX}.")
  endif()
endif()